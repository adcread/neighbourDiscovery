...
for levelIndex = 1:noSuppressionLevels
    ...
    pattern{levelIndex} = zeros(size(azGrid));
    
    % Calculate the lvel of sidelobe suppression to apply
    ...
    for antennaIndex = 1:noElements
        ...
        for elIndex = 1:length(el)
            ...
            for azIndex = 1:length(az)
                ...
                pattern{levelIndex}(elIndex,azIndex) = pattern{levelIndex}(elIndex,azIndex) + (weights(antennaIndex) * exp(-1i * waveNumber * (antennaIndex-1) * antennaSeparation * cos(el(elIndex)) * sin(az(azIndex))));
